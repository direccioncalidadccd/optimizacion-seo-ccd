...
export default function FormProm() {
  ...
  const ValidateField = (name: string, value: string) => {
    switch (name) {
      ...
      case "movil":
        return /^[0-9]{9}$/.test(value.trim()) ? "" : "Número movil invalido";
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(
        `${environment.baseUrl}/inicio/EnviarCorreoPromo`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="  p-4 md:p-8 bg-sectors-PromoForm">
      <div className="container mx-auto">
        <div className="relative flex flex-col justify-start w-[40%] max-sm:w-full max-lg:w-[60%] rounded-xl  p-8 gap-4 ">
          {/* Form Section */}
          <div className="flex flex-col justify-center items-center">
            <h1 className=" text-[2.5rem] max-sm:text-xl font-bold text-colors-cyan-ccd">
              ¿Quieres conocer
            </h1>
            <h2 className=" text-[2.5rem] max-sm:text-xl font-bold text-white mb-6 neon-white">
              MAS PROMOCIONES??
            </h2>
          </div>
          <p className="text-gray-200 text-center text-lg mb-8">
            Déjanos tus datos para enviarte mas promociones.
          </p>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  {errors.movil && (
                    <p className="text-red-500 text-sm mt-1">{errors.movil}</p>
                  )}
                </div>
