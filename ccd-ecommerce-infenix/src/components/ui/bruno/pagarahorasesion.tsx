...
interface Props {
  ...
  productocurso: Array<any>;
}

export default function PagarAhoraSesionComponent({
  array,
  precio,
  productocurso,
  precioanterior,
  ...
  const enviarformulario = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    <body>
        <div class="container">
            <header class="header">
                <img src="https://ccdcapacitacion.edu.pe/wp-content/uploads/2022/10/PLANTILLA-WEB-CCD.png"  class="logo">
            </header>
  ...
  return (
    <>
      <div className="flex flex-col gap-2">
        <div className="  ">
          <button onClick={onOpen1} className="bg-transparent !p-0 text-white">
            <h4>Comprar Ahora</h4>
          </button>
        </div>
        <div className="flex gap-2 items-center">
          {productocurso?.length > 0 ? (
            <>
              <button
                onClick={onOpen1}
                className="text-3xl bg-colors-cyan-ccd rounded-xl p-3 py-[2px] font-bold text-colors-dark-blue-ccd"
              >
                S/.{precio}
              </button>
              {precioanterior !== undefined && (
                <div className="text-lg line-through text-white decoration-colors-cyan-ccd">
                  S/. {Math.round(precioanterior)}
                </div>
              )}
            </>
          ) : (
